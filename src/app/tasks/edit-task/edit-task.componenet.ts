...
export class EditTaskComponent implements OnInit {
  ...
  showDatePickerReminderDate : boolean = false;
  showReminderDateField : boolean = false;
  ...
  reminderDateDate : Date = this.toUTCDate(new Date());
  ...
  createFormControls(task : Task) {
    ...
    this.reminderDateText = new FormControl(task.reminderAt);
  ...
  private applyReminderDate() : void {
    var date = this.dateFormatPipe.transformDate(this.reminderDateDate);
  ...
  changeDueDate(dueDateVal){
    ...
    if(dueDateVal != null && dueDateVal != '' && (!dueDateVal.match(/^(0[1-9]|[12][0-9]|3[01])[\- \/.](?:(0[1-9]|1[012])[\-\/.](19|20)[0-9]{2})$/))){
